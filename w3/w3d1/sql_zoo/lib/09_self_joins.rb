...
def craiglockhart_and_tollcross
  ...
  #Need to add extra join to access stops by name instead of ID!
  execute(<<-SQL)
    SELECT
      a.company,
      a.num
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    FROM
      routes a
    JOIN
      routes to_transfer ON (a.company = to_transfer.company AND a.num = to_transfer.num)
    JOIN
      stops transfer ON (transfer.id = to_transfer.stop_id)
    JOIN
      routes from_transfer ON (transfer.id = from_transfer.stop_id)
    JOIN
      routes b ON (b.company = from_transfer.company AND b.num = from_transfer.num)
    WHERE
      a.stop_id = (
        SELECT
          id
